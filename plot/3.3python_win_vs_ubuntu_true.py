...
path = '../../Ubuntu Result/Python/UMF_PACK TRUE/'
...
path = '../../Windows Result/Python/UMFPACK = true/'
j = 0
for name in files:
    perc = path + name + '_true.txt'
    df = pd.read_table(perc, delim_whitespace=True, skiprows=1, header=None)
...
plt.yscale('log')
plt.title('Python Ubuntu vs Python Windows with UMF_PACK TRUE')
plt.ylabel('Memory')
